...
class Multinominal extends Controller
{
   ...
   */
   ...
   public function seluruhKata($kelas)
   {
      $kata = $this->model('NiveBayesModel')->getSeluruhKata($kelas);
   ...
   public function F_multinominal($kata)
   {
      $data = $kata;
      if (isset($data)) {
         $kelas = ['cyberbullying','non_cyberbullying'];
         for ($i=0; $i < count($kelas); $i++) { 
            // $cek = $this->getFrekuensi($data, "$kelas[$i]");
            if ($this->getkata($data, "$kelas[$i]") == false){
               $frekuensi = 0;
            }else{
               $frekuensi = $this->getkata($data, "$kelas[$i]");
            }
            $smoothing = 1;
            $kondisi1 = $frekuensi + $smoothing;
   ...
   /*
   * Akhir conditional Probability
   */

   public function penentuanKelasMultinominal($conditionalProbability)
   {
      for ($i=0; $i < 2; $i++) { 
         if ($i == 0) {
